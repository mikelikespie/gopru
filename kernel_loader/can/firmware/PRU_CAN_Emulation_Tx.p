...
//	MOV32  	RegisterValue, PRU_CAN_TIMING_VAL_TX
//	MOV32  	RegisterOffset,0x4E0 //8 MS delay as default value
...
POLL_BUS_OFF:	
	...
	QBGT		POLL_BUS_OFF,  RegisterValue.w0, 11
	ADD			RegisterValue.w2, RegisterValue.w2, 1
	QBGT		TRANSMIT_POLL_BUS_OFF, RegisterValue.w2, 128
		
TRANSMIT_BUS_OFF_RESET:
	MOV32  		ShiftRegister,PRU_CAN_TX_ERROR_COUNTER_REGISTER
	XOR         RegisterValue, RegisterValue,RegisterValue
	SBBO   		RegisterValue,ShiftRegister,0,4	
...
ARM_DSP_INTR_POLL:
	...
	MOV32  		ShiftRegister,PRU_CAN_TX_ERROR_COUNTER_REGISTER
	LBBO   		RegisterOffset,ShiftRegister,0,4	
	XOR         RegisterValue, RegisterValue,RegisterValue
	QBLT		TRANSMIT_POLL_BUS_OFF,RegisterOffset,254
	...
	TRANSMIT_POLL:	
	MOV32  		ShiftRegister,PRU_CAN_TX_BUS_AQUIRED			
	//MOV32  		r2,0
	XOR	        RegisterValue, RegisterValue, RegisterValue
	SBBO		RegisterValue, ShiftRegister,0,4
...
ARM_DSP_INTERRUPT:
	MOV32     	RegisterOffset, 0x01c34284
...
/////////////////////////////////////////////////////////////////////////////////////////
PRU_CAN_TRANSMIT:							
	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX0_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4		
	QBBC   		MAILBOX0_TRANSMIT,RegisterValue,0

	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX1_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4
	QBBC   		MAILBOX1_TRANSMIT,RegisterValue,0

	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX2_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4
	QBBC   		MAILBOX2_TRANSMIT,RegisterValue,0

	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX3_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4
	QBBC   		MAILBOX3_TRANSMIT,RegisterValue,0

	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX4_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4
	QBBC   		MAILBOX4_TRANSMIT,RegisterValue,0

	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX5_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4
	QBBC   		MAILBOX5_TRANSMIT,RegisterValue,0

	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX6_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4
	QBBC   		MAILBOX6_TRANSMIT,RegisterValue,0

	MOV32  		ShiftRegister,PRU_CAN_TX_MAILBOX7_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4
...
CAN_FRAME_RDY_TO_SEND:
	...
	MOV32  		ShiftRegister,PRU_CAN_TIMING_VAL_TX			
//	MOV32  		DelayCntr,0
	LBBO		DelayCntr,ShiftRegister,0,4
//	LBBO		DelayCntr.w2,ShiftRegister,2,2						
//==================================================================================================================
...
TIMER_CONFIG:
	...
	MOV     	RegisterValue, 0x00000002
	SBBO      	RegisterValue.w0, RegisterOffset, TMR_INTCTL_OFFSET,  2

	// Enable the Timer in auto reload-shot mode
	LDI       	RegisterValue.w0, 0x0080
	SBBO      	RegisterValue.w0, RegisterOffset, TMR_TCR_OFFSET,  2
	...
	// Read setup delay
	MOV32     	RegisterValue, PRU_CAN_TIMING_VAL_TX_SJW
	LBBO	  	DelayCntr, RegisterValue, 0, 4	
...
CHECK_CAN_BUS_IDLE_LOOP:			
	QBBC		CHECK_CAN_BUS_IDLE, r31, 18	
	ADD  		RegisterValue.w0,RegisterValue.w0,1
	CALL		TIMER_EVENT_WAIT
...
CAN_BUS_IDLE:		
	...
	MOV32  		RegisterOffset,1
	SBBO		RegisterOffset,RegisterValue,0,4


//	GPIO_SET 	21	  //debug code
	XOR         CRC_TempVar, CRC_TempVar, CRC_TempVar  
...
BIT_ONE_SETUP_DELAY:
	...
	// Moniter the CAN Bus, if it being driven low, CRC_Input.w0=0xff -> Error
...
//	GPIO_CLEAR	21					// debug Code
	CALL        TIMER_EVENT_WAIT
	MOV			CRCFrame.b1, 2      // Wait for ACK Bit for Three bit durations
...
FRAME_ACK_SUCCESS:				
	GPIO_SET	GPIO_TX_BIT
	MOV  		BitStuffCounter.w0,7
	MOV32  		ShiftRegister,PRU_CAN_TX_INTERRUPT_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4	
	CLR    		RegisterValue, RegisterValue,14
    SBBO   		RegisterValue,ShiftRegister,0,4
	...
	QBNE        CAN_FRAME_EOF_STATE,RegisterValue,127
	
	MOV32       RegisterOffset,PRU_CAN_RX_ERROR_COUNTER_REGISTER
    LBBO        RegisterValue,RegisterValue,0,4	
	QBGE        CAN_FRAME_EOF_STATE,RegisterValue,127
...
CAN_FRAME_EXIT_ANALYSIS:
	...
	// STD Can Frame Remove subtract the Start of Frame bit,check for ARBTR_FAILED
	SUB         CRC_Input.w2,CRC_Input.w2,1            
	QBGT		CAN_ARBTR_FAILED,CRC_Input.w2,11			
	// No ARBTR Failure, bit error
	MOV32		CanCurrentState,CAN_BIT_ERROR
	JMP			CAN_FRAME_EXIT
	
	// EXTD Can Frame Remove subtract the SOF+SRR+EDI bit,check for ARBTR_FAILED	
CAN_TRANS_EXTD_FRM_EXIT:
	SUB         CRC_Input.w2,CRC_Input.w2,3
	QBGT		CAN_ARBTR_FAILED,CRC_Input.w2,29
	// No ARBTR Failure, bit error	
	MOV32		CanCurrentState,CAN_BIT_ERROR
	JMP			CAN_FRAME_EXIT

	// CAN ARBTR FAILED, reset PRU_CAN_TX_BUS_AQUIRED flag, retrycnt is less than 3,jmp CHECK_CAN_BUS_IDLE 
CAN_ARBTR_FAILED:									
	MOV32  		ShiftRegister,PRU_CAN_TX_BUS_AQUIRED			
	XOR  		RegisterValue,RegisterValue, RegisterValue
	SBBO		RegisterValue,ShiftRegister,0,4
	...
	QBGE		CHECK_CAN_BUS_IDLE,CRCFrame.b0,3
	XOR			RegisterValue, RegisterValue, RegisterValue
	SET			RegisterValue,	RegisterValue, 4
	JMP			UPDATE_ARM_DSP 

	// Update the Errror Counter
CAN_FRAME_EXIT:	
	MOV			CRCFrame.b0,0
	MOV32  		ShiftRegister,PRU_CAN_TX_ERROR_COUNTER_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4	
	ADD			RegisterValue,RegisterValue,8  // count should be 8 *******
	SBBO   		RegisterValue,ShiftRegister,0,4
	
	QBLT		BUS_OFF,RegisterValue,255
	QBLT		ERR_PASSIVE,RegisterValue,128
...
//	LBBO      	RegisterValue, RegisterOffset, 0,  4
//	SET         RegisterValue, RegisterValue, 4
//	CLR         RegisterValue, RegisterValue, 3
//	SBBO		RegisterValue, RegisterOffset,0, 4
	JMP			EXIT_ERROR
...
ERR_PASSIVE:
	...
	MOV32  		ShiftRegister,PRU_CAN_TX_BUS_AQUIRED			
	XOR  		RegisterOffset,RegisterOffset, RegisterOffset
	SBBO		RegisterOffset,ShiftRegister,0,4	
...
TX_PASSIVE_ERR_FRAME:			
	GPIO_SET	GPIO_TX_BIT   // Generate Error Frame
	CALL		TIMER_EVENT_WAIT	
	ADD			RegisterValue, RegisterValue, 1
	QBGT    	TX_PASSIVE_ERR_FRAME, RegisterValue, 6
	JMP			ERROR_DELIMITER
...
//	GPIO_SET	GPIO_TX_BIT   
	CALL		TIMER_EVENT_WAIT	
	ADD     	RegisterValue, RegisterValue, 1
	QBGT    	TX_ACTIVE_ERR_FRAME, RegisterValue, 6
...
ERROR_DELIMITER_FRAME_LOOP:
	GPIO_SET	GPIO_TX_BIT   // Generate Error  Delimiter Frame
	XOR       	CRC_Input.w0, CRC_Input.w0, CRC_Input.w0
	CALL		POLL_CAN_BUS_STATUS
...
NO_DELIMITER_ERR:
	ADD     	RegisterValue, RegisterValue, 1
	QBGT    	ERROR_DELIMITER_FRAME_LOOP, RegisterValue, 8		
...
EXIT_ERROR:
	MOV32  		ShiftRegister,PRU_CAN_TX_INTERRUPT_STATUS_REGISTER
	LBBO   		RegisterValue,ShiftRegister,0,4	
	SET    		RegisterValue, RegisterValue,14
...
POLL_CAN_BUS_STATUS:
	MOV	     	r1.w2, INTCTL_BASE 
	MOV		 	r1.w0, SECR1_OFFSET											
	
CAN_BUS_POLL:	
	QBBS	 	CAN_BUS_SET, r31, 18
...
TIMER_EXPIERED:	
	LBBO     	r0, r1, 0, 4
	QBBC	 	CAN_BUS_POLL, r0, TIMER_SYS_EVT
				
	MOV32    	r1, TMR_REGS
	MOV     	r0, 0x00000003
	SBBO    	r0, r1, TMR_INTCTL_OFFSET,  2
				
	// Clear Timer interrupt
	MOV32     	r0, (0x00000000 | TIMER_SYS_EVT)
...
TIMER_EVENT_HANDLER:
	...
	LBBO     	r0, r1, 0, 4
	QBBC	 	TIMER_EVENT_WAIT_LOOP, r0, TIMER_SYS_EVT
