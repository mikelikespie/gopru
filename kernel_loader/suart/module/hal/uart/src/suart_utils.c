			...
			arm_pru_iomap * pru_arm_iomap)
{
	...
	if ((SUART_8X_OVRSMPL == oversampling) || (0 == oversampling)){
		mcasp0Regs->RMASK = 0x000000FF; //0x0000FFFF for 16
		mcasp0Regs->RFMT = 0x0000A038;	//slot size 8 bits  , RPAD = 1. 0x0000A078 for x16
	}
	...
	mcasp0Regs->AFSRCTL = 0x00000012;	//burst mode
	...
	//Configure PDIR for test purposes
	//set frame sync, clocks as output, AXR8, 10 & 14 as output, AXR7, 9 & 13  
	mcasp0Regs->PDIR = MCASP_PDIR_VAL;	// has receiver configuration as well            
	mcasp0Regs->PDOUT = 0xFFFF;
...
void suart_mcasp_psc_enable(unsigned int psc1_addr)
{
	...
	if (CSL_FEXT(PSC->MDSTAT[CSL_PSC_MCASP0], PSC_MDSTAT_STATE) ==
	    CSL_PSC_MDSTAT_STATE_ENABLE)
		return;

	// Perform transition
	CSL_FINST(PSC->MDCTL[CSL_PSC_MCASP0], PSC_MDCTL_NEXT, ENABLE);
	...
	while (CSL_FEXT(PSC->MDSTAT[CSL_PSC_MCASP0], PSC_MDSTAT_STATE) !=
	       CSL_PSC_MDSTAT_STATE_ENABLE) ;
}
...
void suart_mcasp_psc_disable(unsigned int psc1_addr)
{
	...
	// Perform transition
	CSL_FINST(PSC->MDCTL[CSL_PSC_MCASP0], PSC_MDCTL_NEXT, SYNCRST);
	CSL_FINST(PSC->PTCMD, PSC_PTCMD_GO0, SET);
